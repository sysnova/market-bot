...
def validate_strategy(implementation: str, source: StrategySource) -> None:
    if implementation in {"1.12.0", "1.13.0"}:
        implementation = "1.11.0"
    if implementation not in {"1.4.0", "1.5.0", "1.8.0", "1.9.0", "1.10.0", "1.11.0"}:
        return
    ...
    if implementation in {"1.9.0", "1.10.0", "1.11.0"}:
        ...
        for name in ("recovery_acceptance_bars", "short_rvol_sessions"):
            behavior.positive_int(name)
    if implementation == "1.11.0":
        if behavior.decimal("recovery_minimum_rvol") <= 0:
            raise ValueError("recovery_minimum_rvol must be positive")
        behavior.positive_int("recovery_rvol_sessions")
        behavior.positive_int("recovery_resistance_sessions")
...
def configure_engine(
    ...
    if implementation == "1.11.0":
        kwargs["recovery_minimum_rvol"] = behavior.decimal("recovery_minimum_rvol")
        for name in ("recovery_rvol_sessions", "recovery_resistance_sessions"):
            kwargs[name] = behavior.positive_int(name)
    return args, kwargs
