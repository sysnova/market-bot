from decimal import Decimal
from pathlib import Path

from app.long_portfolio_engine import load_long_portfolio_policy


def test_production_artifact_preserves_capital_exclusions_and_cash() -> None:
    root = Path(__file__).parents[3]
    policy = load_long_portfolio_policy(
        root / "configs/rules/long_portfolio/1.0.0.yaml"
    )

    assert policy.portfolio_capital_usd == Decimal("103000")
    assert policy.cash_weight_percent == Decimal("11.20")
    assert set(policy.excluded_symbols) == {
        "JASMY", "ANF", "ADA", "ASPN", "ASTI", "RR", "SIDU"
    }
    assert policy.allocation_for("HIMS").weight_percent == Decimal("11.73")  # type: ignore[union-attr]
    assert policy.unallocated_weight_percent == Decimal("0")
