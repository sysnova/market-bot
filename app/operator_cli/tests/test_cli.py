...
def test_assembly_command_exposes_implementation_strategy_and_mode() -> None:
    ...
    assert payload["engines"]["entry-opportunity"]["implementation"] == "6.0.0"
    assert payload["engines"]["swing-trade"]["implementation"] == "1.3.0"
...
def test_runtime_plan_command_exposes_commands_and_dependency_batches() -> None:
    ...
    assert result.exit_code == 0
    payload = json.loads(result.stdout)
    assert payload["definition_version"] == "7.30.0"
