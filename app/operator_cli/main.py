...
@engine.command("leveraged-thesis")
def leveraged_thesis_process(
    ready_path: Annotated[
        Path, typer.Option(help="Readiness file for leveraged thesis alerts.")
    ] = Path(".runtime/status/leveraged-thesis.ready.json"),
) -> None:
    """Fuse underlying structure with SIP flow for directional instruments."""

    from app.integration.leveraged_thesis_composition import run_leveraged_thesis_process

    _run_async(run_leveraged_thesis_process(ready_path=ready_path))
