...
class InvalidSubjectError(ValueError):
    """Raised when a publish subject or subscription pattern is malformed."""


def validate_publish_subject(subject: str) -> None:
    tokens = _tokens(subject)
    if "*" in tokens or ">" in tokens:
        raise InvalidSubjectError("publish subjects cannot contain wildcards")
...
def subject_matches(pattern: str, subject: str) -> bool:
    """Return whether a concrete subject matches a NATS wildcard pattern."""
