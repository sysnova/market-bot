"""Durable pending SHORT intents; independent of disposable engine cache views."""
...
from app.leveraged_thesis_engine.v11 import (
    DeferredShortState,
...
class ShortStateStore(Protocol):
    async def get(self, symbol: str) -> DeferredShortState: ...
    async def put(self, symbol: str, state: DeferredShortState) -> None: ...


class RedisShortStateStore:
    def __init__(self, redis: Redis, namespace: str) -> None:
        self.redis, self.namespace = redis, namespace + "pending-short:v1:"

    async def get(self, symbol: str) -> DeferredShortState:
        raw = await asyncio.to_thread(self.redis.get, self.namespace + symbol)
        return (
            DeferredShortState.model_validate_json(cast(str | bytes, raw))
            if raw
            else DeferredShortState()
        )
...
class DeferredShortRuntime:
    def __init__(
        self,
        engine: LeveragedThesisEngineV11,
        publisher: EventPublisher,
        store: ShortStateStore,
        clock: Clock | None = None,
    ) -> None:
        self.engine, self.publisher, self.store = engine, publisher, store
        self.clock = clock or SystemClock()
        self.states: dict[str, DeferredShortState] = {}
        self.lock = asyncio.Lock()

    async def restore(self) -> None:
        async with self.lock:
            for pair in self.engine.pairs:
                self.states[pair.underlying_symbol] = await self.store.get(pair.underlying_symbol)
        await self.tick()
    ...
    async def handle(self, envelope: EventEnvelope) -> None:
        ...
        async with self.lock:
            for pair in self.engine.pairs:
                if symbol not in {pair.underlying_symbol, pair.bearish_instrument}:
                    continue
                old = self.states.get(pair.underlying_symbol, DeferredShortState())
                state = self.engine.advance_short(old, now=self.clock.now(), alert=alert, flow=flow)
                await self._save_and_publish(pair.underlying_symbol, state)
            ...
            for symbol, old in tuple(self.states.items()):
                if (
                    ...
                    await self._save_and_publish(
                        symbol, self.engine.advance_short(old, now=self.clock.now())
                    )
                else:
                    await self._save_and_publish(symbol, old)

    async def _save_and_publish(self, symbol: str, state: DeferredShortState) -> None:
        ...
        if item is None or state.intent is None:
            return
        signature = f"{state.intent.alert.alert_id}:{item.state}:{item.reasons}"
        ...
        await self.publisher.publish(
            ...
            EventEnvelope(
                ...
                subject=symbol,
                payload=item,
                causation_id=state.intent.alert.alert_id,
            ),
        )
        ...
        if signal is not None:
            signal = signal.model_copy(
                update={
                    ...
                    "setup_id": f"leveraged-short:{state.intent.alert.alert_id}",
                    "source_event_ids": tuple(
                        dict.fromkeys((*signal.source_event_ids, state.intent.alert.alert_id))
                    ),
                }
            )
