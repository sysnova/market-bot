    ...
    *,
    ...
    if settings.entry_watcher_enabled:
        try:
            ...
            session_factory = create_session_factory(entry_watch_database)
            entry_watch_store = PostgresEntryWatchStore(session_factory)
            entry_opportunity_store = PostgresEntryOpportunityStore(session_factory)
            if await entry_watch_store.is_ready():
                ...
                if await entry_opportunity_store.is_ready():
                    ...
                    await logger.awarning(
                        "entry_opportunity_schema_unavailable",
                        migration="20260807010000_entry_opportunity_lifecycle.sql",
                    )
            else:
                await logger.awarning(
    ...
    runtime = AnalysisRuntime(
        ...
        publisher=publisher,
        long_term=assembly.build_long_term(),
    ...
    try:
        ...
        if once:
            result: dict[str, Any] = {
                ...
                "engine_implementations": {
                    slot.value: assembly.spec(slot).implementation
                    for slot in (
        ...
        if capture_subscription is not None:
            await capture_subscription.unsubscribe()
