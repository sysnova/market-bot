...
class PortfolioFlowEngine:
    """Keep only a short rolling window and emit human PROTECT alerts."""
    ...
    def __init__(self, policy: PortfolioFlowPolicy | None = None) -> None:
        ...
        self._trades: dict[str, deque[_Trade]] = defaultdict(deque)
    ...
    def ingest(self, envelope: EventEnvelope, *, now: datetime) -> LocalAlert | None:
        ...
        if envelope.event_type == "market.quote.received":
            self._quotes[symbol] = (
                Decimal(str(payload["bid_price"])), Decimal(str(payload["ask_price"]))
            )
            return None
        ...
        bid, _ask = self._quotes[symbol]
        trades = self._trades[symbol]
        trades.append(_Trade(envelope.occurred_at, price, size, price <= bid))
    ...
    def _evaluate(
        self, symbol: str, trades: deque[_Trade], now: datetime
    ) -> LocalAlert | None:
        ...
        if (
            ...
            return None
        previous = self._last_alert.get(symbol)
