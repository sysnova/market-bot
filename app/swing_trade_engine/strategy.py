...
def validate_strategy(implementation: str, source: StrategySource) -> None:
    ...
    if implementation in {"1.6.0", "1.7.0", "1.8.0", "1.9.0"}:
        for name in (
            "macd_fast_period",
            "macd_slow_period",
            "macd_signal_period",
    ...
    if implementation in {"1.7.0", "1.8.0", "1.9.0"}:
        for name in (
        ...
        for name in ("rebound_stop_buffer", "minimum_progress_r"):
            if behavior.decimal(name) <= 0:
                raise ValueError(f"SwingTrade {name} must be positive")
    if implementation in {"1.8.0", "1.9.0"} and behavior.positive_int("rebound_rising_closes") < 2:
